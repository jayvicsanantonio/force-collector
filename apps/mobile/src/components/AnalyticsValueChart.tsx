...
type AnalyticsValueChartProps = {
  points: AnalyticsValuePoint[];
  height?: number;
  accessibilityLabel?: string;
};

export function AnalyticsValueChart({
  points,
  height = 140,
  accessibilityLabel = "Collection value chart",
}: AnalyticsValueChartProps) {
  ...
  const chart = useMemo(() => {
    ...
    const coords = sorted.map((point, index) => toPoint(point.value, index));
  ...
  if (!points.length) {
    return (
      <View className="items-center justify-center rounded-xl border border-dashed border-hud-line/70 bg-raised-surface/40 px-4 py-6">
        <Text className="text-xs text-secondary-text">Pricing unavailable.</Text>
      </View>
    );
  }
    ...
    >
      {width > 0 && chart ? (
        <Svg width={width} height={height}>
          ...
          <Path d={chart.areaPath} fill="url(#valueGradient)" />
          <Polyline
